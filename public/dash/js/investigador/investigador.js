...
if($("#newInvestigador").length>=1){
    function validateEditor() {
        // Revalidate the content when its value is changed by description
        $('#summernote').formValidation('revalidateField', 'content');
    }
    $('#newInvestigador').formValidation({
        framework: 'bootstrap',
        icon: {
        ...
        fields: {
            name: {
                validators: {
                    ...
                    stringLength: {
                        min: 10,
                        max: 77,
                        message: 'El nombre debe ser mayor de 10 y menor de 77 caracteres de longitud.'
                    }
                }
            },
            permalink: {
            ...
            image: {
                validators: {
                    notEmpty: {
                        message: 'La imagen es campo necesario y no puede estar vacío.'
                    }
                }
            },
            summernote: {
                validators: {
                    callback: {
                        ...
                        callback: function(value, validator, $field) {
                            ...
                            // <p><br></p> is code generated by description for empty content
        ...
        $.ajax({
            ...
            success : function(response){
                if(response.message=="SUCCESS" && response.code==200){
                    ...
                    setTimeout(function(){
                        window.location = "/dashboard/investigador";
                    },3000);
...
if($("#name").length>=1){
    $("#name").change(function(){
        ...
        $.ajax({
            url : "validateurl",
            type : "POST",
            data : {name:name,key: $('#key-security').attr('data-key'),value: $('#value-security').attr('data-value')},
            dataType : "json",
            success : function(response){
                if(response.message=="SUCCESS" && response.code==200){
                    $("#note-permalink").val(response.new_url);
                    $('#newInvestigador').formValidation('revalidateField', 'permalink');
                }else{
...
/* Update Note */
if($("#updateInvestigador").length>=1){
    function validateEditor() {
        // Revalidate the content when its value is changed by description
        $('#summernote').formValidation('revalidateField', 'content');
    }
    $('#updateInvestigador').formValidation({
        framework: 'bootstrap',
        icon: {
            valid: 'glyphicon glyphicon-ok',
            invalid: 'glyphicon glyphicon-remove',
